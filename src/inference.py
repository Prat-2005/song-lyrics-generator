...
def load_model(checkpoint_path: str, device: torch.device) -> Tuple[LyricLSTM, Dict[str, int], Dict[int, str], Dict[str, Any]]:
    ...
    Returns:
        Tuple containing:
        - model: The loaded LyricLSTM model in evaluation mode.
    ...
    hidden_dim = checkpoint.get("hidden_dim", 512)  # Default to 512
...
def generate_text(model: LyricLSTM, word2idx: Dict[str, int], idx2word: Dict[int, str],
                  prompt: str, max_words: int, temperature: float, top_k: int, device: torch.device) -> str:
    """Generate text using the trained model.

    Args:
        model: The trained LyricLSTM model.
        word2idx: Dictionary mapping words to indices.
        idx2word: Dictionary mapping indices to words.
